...
CASES_FOLDER = '/cases'
PROCESSOR_FOLDER = os.path.join(CASES_FOLDER, 'processor')
MALWARE_FOLDER = os.path.join(CASES_FOLDER, 'malware')
    ...
    if request.method == 'POST':
        ...
        if file and destination:
            ...
            if destination == 'Triage':
                dest_path = os.path.join(PROCESSOR_FOLDER, file.filename)
            elif destination == 'Malware':
                dest_path = os.path.join(MALWARE_FOLDER, file.filename)
            else:
...
@app.route('/browse', defaults={'req_path': 'processor'})
    ...
    if request.method == 'POST':
        try:
            ...
            scale_cmd = [
                'docker', 'compose',
                '-f', compose_file,
                'up', '--scale', f'titan={replicas}', '-d'
            ]

            result = subprocess.run(scale_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            if result.returncode != 0:
                raise RuntimeError(result.stderr.strip())

            message = f'Successfully scaled TITAN with {replicas} worker container(s).'
            log_action(f'Scaled TITAN to {replicas} worker container(s)')
        except Exception as e:
            message = f'Error scaling containers: {str(e)}'

    return render_template('scale.html', message=message)
...
if __name__ == '__main__':
    ...
    socketio.run(app, debug=True, host='0.0.0.0', port=5111, allow_unsafe_werkzeug=True)
