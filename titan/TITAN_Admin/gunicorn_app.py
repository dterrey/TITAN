...
# Define paths to TITAN_IOC and TITAN Console
TITAN_IOC_PATH = '/home/titan/Downloads/TITAN/TITAN_IOC'
TITAN_CONSOLE_PATH = '/home/titan/Downloads/TITAN/TITAN_Admin/TITAN_CONSOLE'

# Start TITAN_IOC using Gunicorn asynchronously
def start_titan_ioc():
    try:
        print("Starting TITAN_IOC...")
        proc = subprocess.Popen(
            ['gunicorn', '-w', '4', '-b', '0.0.0.0:5112', 'app:app'],
            cwd=TITAN_IOC_PATH,
            stdout=subprocess.PIPE,  # Capture output
            stderr=subprocess.PIPE,
            shell=False
        )
        print(f"TITAN IOC started on port 5112 with Gunicorn. PID: {proc.pid}")
        return proc
    except Exception as e:
        print(f"Error launching TITAN IOC: {str(e)}")

# Start TITAN Console using Gunicorn asynchronously
def start_titan_console():
    try:
        print("Starting TITAN Console...")
        proc = subprocess.Popen(
            ['gunicorn', '-w', '4', '-b', '0.0.0.0:5232', 'app:app'],
            cwd=TITAN_CONSOLE_PATH,
            stdout=subprocess.PIPE,  # Capture output
            stderr=subprocess.PIPE,
            shell=False
        )
        print(f"TITAN Console started on port 5232 with Gunicorn. PID: {proc.pid}")
        return proc
    except Exception as e:
...
# The main Flask app entry point
if __name__ == '__main__':
    print("Master app is running.")

    # Start TITAN_IOC before the main Flask app
    print("Starting TITAN_IOC from master app...")
    titan_ioc_process = start_titan_ioc()

    # Start TITAN Console before the main Flask app
    print("Starting TITAN Console from master app...")
    titan_console_process = start_titan_console()

    # Ensure the subprocesses are running before starting the main app
    if titan_ioc_process.poll() is None and titan_console_process.poll() is None:
        print("Both TITAN_IOC and TITAN Console started successfully.")
    else:
        print("Error: One of the subprocesses failed to start.")

    # Now run the main app using Gunicorn
    print("Starting master app on Gunicorn at port 5111.")
    subprocess.run(['gunicorn', '-w', '4', '-b', '0.0.0.0:5111', 'app:app'])
